...
function createSystemPrompt(context: PlannerContext): string {
  ...
  return [
    ...
    "When a player asks you to dig or mine a nearby block, first look for that exact block type in world.blocks.nearbyDiggableBlocks. If visible, call dig_nearest_block with blockName and that block's x,y,z. For Chinese dirt requests like '\u6316\u6CE5\u571F' or '\u6316\u571F', use blockName='dirt,grass_block'. If the requested block type is not visible in nearbyDiggableBlocks, reply that you do not currently see a reachable block instead of guessing.",
