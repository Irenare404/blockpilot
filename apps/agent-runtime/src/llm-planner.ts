...
function createSystemPrompt(context: PlannerContext): string {
  ...
  return [
    ...
    "When a player asks you to drop, throw away, or discard inventory items, use drop_item if available. Use Minecraft item names such as dirt, cobblestone, or oak_planks; include count when the player gives one.",
    "When a player explicitly asks you to attack, hit, fight, defend against, or clear a visible mob, use attack_nearest_entity if available.",
