...
function createSystemPrompt(context: PlannerContext): string {
  ...
  return [
    ...
    "For area digging or gathering requests like 2x2, 3x3, or 16x16, set dig_nearest_block areaSize to that number. areaSize must be 1..16. Use maxDistance up to 128 only when the target is visible in the snapshot or the player explicitly asks for farther resources.",
    "For Chinese dirt requests like '\u6316\u6CE5\u571F' or '\u6316\u571F', prefer the visible block.name from nearbyDiggableBlocks; old servers may report Grass Block as blockName='grass'. If no exact visible name is available, use blockName='dirt,grass,grass_block'. For tree/wood requests like '\u780D\u6811', '\u780D\u6728\u5934', or 'chop tree', prefer the visible log block's exact name; old servers may report tree trunks as blockName='log'. Do not expand tree requests into a long list of oak_log/spruce_log names when the snapshot shows 'log'.",
    ...
    "When a player asks you to inspect storage, use inspect_nearest_container if available and include the chosen container's x,y,z from world.blocks.nearbyContainers when visible. Do not move items for inspection.",
    "When a player explicitly asks you to put/store/deposit items into a chest or container, use deposit_item_to_container if available and include the container x,y,z when visible.",
    "When a player explicitly asks you to take/withdraw/get items from a chest or container, use withdraw_item_from_container if available and include itemName plus the container x,y,z when visible.",
    ...
    "When a player asks you to craft an item, use craft_item if available. Use Minecraft item names such as oak_planks, crafting_table, stick, wooden_axe, or stone_pickaxe.",
    "When a player asks you to build a simple house/platform/pillar and build_preset is available, use build_preset with preset starter_hut, platform, or pillar. If required materials are missing, first plan short steps to collect logs or dirt and craft planks instead of claiming the build is done.",
    "For building goals, prefer simple staged tasks: collect visible logs with dig_nearest_block, craft planks or a crafting table with craft_item, then call build_preset. Keep each action concrete and confirmable.",
